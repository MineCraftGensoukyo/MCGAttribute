...
public class PhysicalDefenceProcessor extends AbstractAttributeProcessor {
    ...
    public boolean run(EntityLivingBase source, EntityLivingBase target, HashMap<String, Float> vars) {
        float p破甲几率 = AttributeCache.getAttributeValue(source, BuiltInAttributeNames.破甲几率);
        float physics_amount = vars.getOrDefault("physical", 0.0F);
        if (random.nextInt(100) > p破甲几率) {
            // 未能破甲时，计算护甲值
            float p护甲穿透 = AttributeCache.getAttributeValue(source, BuiltInAttributeNames.护甲穿透);
            float p物理护甲 = AttributeCache.getAttributeValue(target, BuiltInAttributeNames.物理护甲);
            physics_amount *= 1 - Math.max(p物理护甲 - p护甲穿透, 0) / 100; // 不能穿到负数
            // 处理物理防御，PVP防御和PVE防御都共用这个属性
            float p物理防御 = AttributeCache.getAttributeValue(target, BuiltInAttributeNames.物理防御);
